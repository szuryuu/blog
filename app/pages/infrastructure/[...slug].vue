...
const { data: page } = await useAsyncData(
  `infrastructure-${route.path}`,
  () => {
    return queryCollection("infrastructure").path(route.path).first();
  },
);
...
if (!page.value) {
  throw createError({
    statusCode: 404,
    statusMessage: "Infrastructure log not found in the database.",
    fatal: true,
  });
}
...
<template>
  <article v-if="page" class="max-w-none pt-8">
    ...
    <header class="mb-20">
      ...
      <h1
        class="font-heading text-6xl sm:text-7xl md:text-8xl text-ghost tracking-wide mb-8 leading-none"
      >
        {{ page.title }}
      </h1>
